...
if ( ! function_exists( 'load_some_file' ) ) :
     ...
     * @param string $ext Name of the extension of the files to be loaded
     */
    function load_some_file( $files = [], $dir = '', $ext = '.php' ) {
...
if ( ! function_exists( 'currency_has_decimal' ) ) {
    ...
    function currency_has_decimals( $currency ) {
        if ( in_array( $currency, ['RIAL', 'SAR', 'HUF', 'JPY', 'TWD'] ) ) {
            return false;
        }
        ...
        if ( empty( $options[$op_name] ) || ! get_post( $options[$op_name] ) ) {
            ...
            if ( $id ) {
                ...
                foreach ( $page_templates as $slug => $name ) {
                    ...
                    if ( in_array( $op_name, ['checkout_page', 'payment_receipt_page'] ) ) break;
...
function atbdp_thumbnail_card( $img_src = '', $_args = [] ) {
 ...
 * @return array
 */
function directorist_prepare_user_favorites( $favorites = [] ) {
...
function directorist_user_notifiable_for( $event_name = '', $user_type = '' ) {
    ...
    if ( ! in_array( $event_name, get_directorist_option( $user_type, [] ), true ) ) {
        return false;
    }
...
function directorist_delete_listing_empty_metadata( $listing_id, array $metadata = [], array $valid_metadata = [] ) {
...
function directorist_download_plugin( array $args = [] ) {
