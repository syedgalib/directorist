...
if ( ! class_exists( 'ATBDP_Tools' ) ) :
    class ATBDP_Tools {
        ...
        public function handle_csv_upload() {
            ...
            add_filter( 'wp_handle_upload_prefilter', [ __CLASS__, 'on_wp_handle_upload_prefilter' ] );

            $file = wp_import_handle_upload();

            remove_filter( 'wp_handle_upload_prefilter', [ __CLASS__, 'on_wp_handle_upload_prefilter' ] );
            ...
            while ( ! $file_object->eof() ) {
                ...
                if ( $tax_inputs ) {
                    foreach ( $tax_inputs as $taxonomy => $value ) {
                        ...
                        $terms = empty( $terms ) ? [] : explode( ',', $post[ $value ] );
                        if ( ! $terms ) {
                            continue;
                        }
